...
public sealed class SelfHostedDb : DbContext
{
    ...
    public static void Initialize(IServiceProvider services)
    {
        ...
        {
            using var tx = db.Database.BeginTransaction();
            if (!HasColumn(db, "plugin_catalog", "Compatibility"))
                db.Database.ExecuteSqlRaw("ALTER TABLE plugin_catalog ADD COLUMN Compatibility TEXT NOT NULL DEFAULT ''");
            if (!HasColumn(db, "plugin_catalog", "VerificationStatus"))
                db.Database.ExecuteSqlRaw("ALTER TABLE plugin_catalog ADD COLUMN VerificationStatus TEXT NOT NULL DEFAULT 'pending'");
            db.SchemaVersions.Add(new SchemaVersionRow { Version = 3 });
            db.SaveChanges();
            tx.Commit();
        }
        if (!db.SchemaVersions.Any(x => x.Version == 4))
        {
            using var tx = db.Database.BeginTransaction();
            if (!HasColumn(db, "sync_items", "Deleted"))
                db.Database.ExecuteSqlRaw("ALTER TABLE sync_items ADD COLUMN Deleted INTEGER NOT NULL DEFAULT 0");
            db.SchemaVersions.Add(new SchemaVersionRow { Version = 4 });
            db.SaveChanges();
            tx.Commit();
        }
    }

    static bool HasColumn(SelfHostedDb db, string table, string column)
    {
