...
public sealed class SelfHostedRuntime : IDisposable
{
    ...
    public static void Configure(ConfigureModel app) => app.services.AddDbContextFactory<SelfHostedDb>(SelfHostedDb.Configure);

    public void Start(InitspaceModel init)
    {
        modpath = Path.Combine(init.path, "SelfHosted");
        Reload();
    ...
    async Task<bool> IdentityMiddleware(bool first, EventMiddleware e)
    {
        ...
        string path = e.httpContext.Request.Path.Value ?? "";
        string method = e.httpContext.Request.Method;
        if (IsPublicUnauthenticatedPath(path, method)) return true;

        var requestInfo = e.httpContext.Features.Get<Shared.Models.Base.RequestModel>();
        if (requestInfo?.IsLocalRequest == true) return true;
        ...
        if (auth == null)
        {
            if (IsProtectedLampaPath(path))
            {
                if (path == "/" || path.EndsWith(".html", StringComparison.OrdinalIgnoreCase))
                    e.httpContext.Response.Redirect("/account/?gate=1");
                else
                {
                    e.httpContext.Response.StatusCode = StatusCodes.Status401Unauthorized;
                    e.httpContext.Response.ContentType = "application/javascript; charset=utf-8";
                    await e.httpContext.Response.WriteAsync("location.replace('/account/?gate=1')");
                }
            }
            else if (e.httpContext.Request.Headers.Accept.Any(x => x?.Contains("text/html", StringComparison.OrdinalIgnoreCase) == true))
                e.httpContext.Response.Redirect("/account/?gate=1");
            else
            {
                e.httpContext.Response.StatusCode = StatusCodes.Status401Unauthorized;
                e.httpContext.Response.ContentType = "application/json; charset=utf-8";
                await e.httpContext.Response.WriteAsJsonAsync(new
                {
                    ok = false,
                    code = "unauthorized",
                    message = "Требуется вход",
                    requestId = e.httpContext.TraceIdentifier
                });
            }
            return false;
        }
    ...
    public static bool IsProtectedLampaPath(string path)
    {
        if (path == "/" || path.EndsWith("/app.min.js", StringComparison.OrdinalIgnoreCase))
            return true;
    ...
    public static bool IsPublicUnauthenticatedPath(string path, string method)
    {
        if (path.Equals("/account", StringComparison.OrdinalIgnoreCase)
            || path.Equals("/account/", StringComparison.OrdinalIgnoreCase)
            || path.Equals("/account/tmdb-logo.svg", StringComparison.OrdinalIgnoreCase)
            || path.StartsWith("/account/pair/", StringComparison.OrdinalIgnoreCase)
            || path.Equals("/api/v1/cub-disabled", StringComparison.OrdinalIgnoreCase)
            || path.Equals("/cub", StringComparison.OrdinalIgnoreCase)
            || path.StartsWith("/cub/", StringComparison.OrdinalIgnoreCase))
            return true;

        if (method.Equals("GET", StringComparison.OrdinalIgnoreCase)
            && (path.Equals("/api/v1/auth/me", StringComparison.OrdinalIgnoreCase)
                || path.Equals("/api/v1/auth/csrf", StringComparison.OrdinalIgnoreCase)
                || (path.StartsWith("/api/v1/auth/pairings/", StringComparison.OrdinalIgnoreCase)
                    && path.EndsWith("/wait", StringComparison.OrdinalIgnoreCase))))
            return true;
        ...
        return method.Equals("POST", StringComparison.OrdinalIgnoreCase)
            && (path.Equals("/api/v1/auth/register", StringComparison.OrdinalIgnoreCase)
                ...
                || path.Equals("/api/v1/auth/reset", StringComparison.OrdinalIgnoreCase)
                || path.Equals("/api/v1/auth/pairings", StringComparison.OrdinalIgnoreCase));
    }
    ...
    void Maintenance()
    {
        try
        {
            ...
            if (lastFeedCheck < DateTime.UtcNow.AddHours(-6))
            {
    ...
    async Task RefreshSubscriptions()
    {
        ...
        try
        {
            ...
            foreach (var target in subscriptions.Select(x => new { x.ContentType, x.TmdbId }).Distinct())
            {
                ...
                string url = $"http://{listenHost}:{CoreInit.conf.listen.port}/tmdb/api/3/{resource}?api_key=4ef0d7355d9ffb5151e987764708ce96&language=ru-RU";
                string json;
                try
                {
                    using var request = new HttpRequestMessage(HttpMethod.Get, url);
                    request.Headers.TryAddWithoutValidation("lcrqpasswd", CoreInit.rootPasswd);
                    using var response = await FeedHttp.SendAsync(request);
