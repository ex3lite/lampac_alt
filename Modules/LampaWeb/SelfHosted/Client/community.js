(function () {
  ...
  function currentLanguage() {
    var language = Lampa.Storage.get('language', 'ru') || 'ru';
    if (language.indexOf('-') > 0) return language;
    return language === 'en' ? 'en-US' : language === 'ru' ? 'ru-RU' : language + '-' + language.toUpperCase();
  }
  ...
  function openFeed() {
    SelfHostedAuth.api('/api/v1/feed').then(function (rows) {
      var root = $('<div style="padding:1.5em;max-width:900px"></div>');
      if (!rows.length) root.append($('<p></p>').text('Новых уведомлений пока нет.'));
      rows.forEach(function (row) {
        var data = {};
        try { data = JSON.parse(row.data || '{}'); } catch (e) {}
        var item = $('<div style="padding:1em 0;border-bottom:1px solid #444"></div>');
        item.append($('<strong></strong>').text(data.message || 'Обновление подписки'));
        item.append($('<div style="opacity:.65"></div>').text(new Date(row.createdAt).toLocaleString()));
        if (!row.readAt) item.append(modalButton('Прочитано', function () {
          SelfHostedAuth.api('/api/v1/notifications/' + row.id + '/read', { method: 'PATCH' }).then(function () { $(this).remove(); }.bind(this));
        }));
        root.append(item);
      });
      Lampa.Modal.open({ title: 'Лента и уведомления', html: root, size: 'large', onBack: function () { Lampa.Modal.close(); } });
    }).catch(function (error) { Lampa.Noty.show(error.message); });
  }

  function setLocalPlugin(plugin, installed) {
    var list = Lampa.Storage.get('plugins', '[]') || [];
    list = list.filter(function (item) { return (typeof item === 'string' ? item : item.url) !== plugin.url; });
    if (installed) list.push({ url: plugin.url, name: plugin.name, status: 1 });
    Lampa.Storage.set('plugins', list, true);
  }

  function openCatalog() {
    SelfHostedAuth.api('/api/v1/catalog/plugins').then(function (plugins) {
      var root = $('<div style="padding:1.5em;max-width:900px"></div>');
      if (!plugins.length) root.append($('<p></p>').text('Администратор пока не опубликовал плагины.'));
      plugins.forEach(function (plugin) {
        var item = $('<div style="padding:1em 0;border-bottom:1px solid #444"></div>');
        item.append($('<h3></h3>').text(plugin.name + (plugin.version ? ' · ' + plugin.version : '')));
        item.append($('<p></p>').text((plugin.description || plugin.author || '') + ' · ★ ' + Number(plugin.rating || 0).toFixed(1) + ' · ' + plugin.installs + ' установок'));
        item.append(modalButton(plugin.installed ? 'Удалить' : 'Установить', function () {
          var install = !plugin.installed;
          SelfHostedAuth.api('/api/v1/catalog/plugins/' + plugin.id + '/install', { method: install ? 'PUT' : 'DELETE' }).then(function () {
            plugin.installed = install; setLocalPlugin(plugin, install); Lampa.Modal.close(); openCatalog();
          });
        }));
        item.append(modalButton('Оценить', function () {
          Lampa.Select.show({ title: 'Оценка плагина', items: [1, 2, 3, 4, 5].map(function (value) { return { title: '★'.repeat(value), value: value }; }), onSelect: function (rating) {
            SelfHostedAuth.api('/api/v1/catalog/plugins/' + plugin.id + '/rating', { method: 'PUT', body: JSON.stringify({ value: rating.value }) }).then(function () { Lampa.Noty.show('Оценка сохранена'); });
          }, onBack: function () { Lampa.Controller.toggle('content'); } });
        }));
        root.append(item);
      });
      Lampa.Modal.open({ title: 'Каталог плагинов', html: root, size: 'large', onBack: function () { Lampa.Modal.close(); } });
    }).catch(function (error) { Lampa.Noty.show(error.message); });
  }

  function smartSearch() {
    var query = prompt('Название, год, жанр или персона');
    if (!query) return;
    SelfHostedAuth.api('/api/v1/search/smart?query=' + encodeURIComponent(query) + '&language=' + encodeURIComponent(currentLanguage())).then(function (parsed) {
      var request = new Lampa.Reguest();
      request.silent(parsed.url, function (data) {
        var results = (data.results || []).filter(function (item) { return item.media_type === 'movie' || item.media_type === 'tv'; }).map(function (item) {
          item.title = item.title || item.name || 'Без названия'; return item;
        });
        if (!results.length) return Lampa.Noty.show('Ничего не найдено');
        Lampa.Select.show({ title: 'Умный поиск', items: results, onSelect: function (item) {
          Lampa.Activity.push({ url: '', component: 'full', id: item.id, method: item.media_type, card: item, source: 'tmdb' });
        }, onBack: function () { Lampa.Controller.toggle('content'); } });
      }, function () { Lampa.Noty.show('TMDB сейчас недоступен'); });
    }).catch(function (error) { Lampa.Noty.show(error.message); });
  }

  function addCommunitySettings() {
    Lampa.SettingsApi.addComponent({ component: 'selfhost_community', name: 'Сообщество' });
    Lampa.SettingsApi.addParam({ component: 'selfhost_community', param: { type: 'button' }, field: { name: 'Лента и уведомления' }, onChange: openFeed });
    Lampa.SettingsApi.addParam({ component: 'selfhost_community', param: { type: 'button' }, field: { name: 'Каталог плагинов' }, onChange: openCatalog });
    Lampa.SettingsApi.addParam({ component: 'selfhost_community', param: { type: 'button' }, field: { name: 'Умный поиск' }, onChange: smartSearch });
  }
  ...
  Lampa.Listener.follow('full', function (event) {
    ...
    if (!box.length || box.find('.selfhost-reviews').length) return;
    ...
    button('Рецензии', function () { openReviews(card, 1, currentLanguage()); });
    box.children().last().addClass('selfhost-reviews');
    button('👍 / 👎', function () {
  ...
  if (window.appready) addCommunitySettings();
  else Lampa.Listener.follow('app', function (event) { if (event.type === 'ready') addCommunitySettings(); });
})();
