...
public sealed class SelfHostedController : BaseController
{
    ...
    static readonly HttpClient PluginHttp = new() { Timeout = TimeSpan.FromSeconds(15) };
    ...
    public ActionResult AccountPage() => Asset("account.html", "text/html; charset=utf-8");
    ...
    public ActionResult PairPage(string secret) => File(Encoding.UTF8.GetBytes(System.IO.File.ReadAllText(Path.Combine(ModInit.modpath, "Assets", "account.html"), Encoding.UTF8).Replace("\"__PAIR_SECRET__\"", Js(secret))), "text/html; charset=utf-8");

    [HttpGet, AllowAnonymous, Route("/selfhost_auth.js")]
    public ActionResult AuthPlugin() => Asset("selfhost_auth.js", "application/javascript; charset=utf-8");

    [HttpGet, AllowAnonymous, Route("/selfhost_sync.js")]
    public ActionResult SyncPlugin() => Asset("selfhost_sync.js", "application/javascript; charset=utf-8");

    [HttpGet, AllowAnonymous, Route("/selfhost_community.js")]
    public ActionResult CommunityPlugin() => Asset("selfhost_community.js", "application/javascript; charset=utf-8");
    #endregion
    ...
    public async Task<ActionResult> Register([FromBody] RegisterRequest? body)
    {
        ...
        var user = new UserRow
        {
            ...
            PasswordHash = SelfHostedSecurity.HashPassword(body.Password, ModInit.conf.password_iterations)
        };
        db.Users.Add(user);
    ...
    {
        string email = SelfHostedSecurity.NormalizeEmail(body?.Email ?? "");
        string throttleKey = (HttpContext.Connection.RemoteIpAddress?.ToString() ?? "unknown") + "|" + email;
        using var db = SelfHostedDb.Create();
        var user = await db.Users.FirstOrDefaultAsync(x => x.EmailNormalized == email);
        if (user == null || user.Status != "active" || !SelfHostedSecurity.VerifyPassword(body?.Password ?? "", user.PasswordHash))
        {
    ...
    public async Task<ActionResult> ResetPassword([FromBody] ResetPasswordRequest? body)
    {
        ...
        var reset = await db.PasswordResets.FirstOrDefaultAsync(x => x.TokenHash == hash && x.UsedAt == null && x.ExpiresAt > DateTime.UtcNow);
        ...
        user.PasswordHash = SelfHostedSecurity.HashPassword(body.NewPassword, ModInit.conf.password_iterations);
    ...
    public async Task<ActionResult> CreatePairing([FromBody] PairingRequest? body)
    {
        ...
        var pairing = new PairingRow
        {
            ...
            ExpiresAt = DateTime.UtcNow.AddMinutes(ModInit.conf.pairing_minutes)
        };
        using var db = SelfHostedDb.Create();
        ...
        await db.SaveChangesAsync();
        string root = string.IsNullOrWhiteSpace(ModInit.conf.public_url) ? host : ModInit.conf.public_url.TrimEnd('/');
    ...
    [HttpGet, AllowAnonymous, Route("/api/v1/auth/pairings/{id}/wait")]
    public async Task<ActionResult> WaitPairing(string id, [FromHeader(Name = "X-Pair-Token")] string tvSecret, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested && DateTime.UtcNow < until)
        {
            ...
            if (pairing.Status == "claimed" && pairing.UserId != null && pairing.ConsumedAt == null)
            {
                ...
                var created = await SelfHostedSecurity.CreateSession(user, HttpContext, pairing.DeviceName);
                SelfHostedSecurity.SetCookie(Response, created.Token);
    ...
    [HttpPost, AllowAnonymous, Route("/api/v1/auth/pairings/claim"), Route("/api/v1/auth/pairings/{claim}/claim")]
    public async Task<ActionResult> ClaimPairing([FromBody] ClaimRequest? body, string? claim = null)
    {
        var auth = await RequireCsrf();
        if (auth.Result != null) return auth.Result;
        string hash = SelfHostedSecurity.TokenHash(claim ?? body?.ClaimSecret ?? "");
    ...
    [HttpDelete, AllowAnonymous, Route("/api/v1/account")]
    ...
    public async Task<ActionResult> SyncBootstrap([FromQuery] long after = 0)
    {
        ...
        var items = rows.Select(x => new { x.Kind, key = x.ItemKey, data = JToken.Parse(x.Data), x.Revision, x.UpdatedAt }).ToList();
    ...
    public async Task<ActionResult> SyncPut([FromBody] SyncPutRequest? body)
    {
        ...
        if (Encoding.UTF8.GetByteCount(data) > 256 * 1024) return Error(413, "item_too_large", "Элемент превышает 256 КБ");
        ...
        try
        {
            ...
            row.Data = data; row.Revision = revision; row.UpdatedAt = DateTime.UtcNow;
    ...
    public async Task<ActionResult> SyncDelete([FromQuery] string kind, [FromQuery] string key)
    {
        ...
        string userId = auth.Auth!.Value.User.Id;
        using var db = SelfHostedDb.Create();
        await db.SyncItems.Where(x => x.UserId == userId && x.Kind == kind && x.ItemKey == key).ExecuteDeleteAsync();
    ...
    public async Task<ActionResult> BackupExport()
    {
        ...
        if (auth == null) return Error(401, "unauthorized", "Требуется вход");
        using var db = SelfHostedDb.Create();
        var rows = await db.SyncItems.AsNoTracking().Where(x => x.UserId == auth.Value.User.Id && x.Kind != "feed_state").ToListAsync();
        var items = rows.Select(x => new { x.Kind, key = x.ItemKey, data = JToken.Parse(x.Data) }).ToList();
        return File(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(new { version = 1, exportedAt = DateTime.UtcNow, items })), "application/json", "lampac-backup.json");
    }
    ...
    public async Task<ActionResult> BackupImport([FromBody] JObject? backup, [FromQuery] bool preview = false)
    {
        ...
        if (preview || Request.Path.Value?.EndsWith("/preview", StringComparison.OrdinalIgnoreCase) == true) return OkData(new { valid = true, items = items.Count });
        using var db = SelfHostedDb.Create();
        using var tx = await db.Database.BeginTransactionAsync();
        await db.SyncItems.Where(x => x.UserId == userId && x.Kind != "feed_state").ExecuteDeleteAsync();
        long rev = 0;
        foreach (var token in items.OfType<JObject>())
        {
            string kind = token.Value<string>("kind") ?? ""; string key = token.Value<string>("key") ?? "";
            if (!SyncKinds.Contains(kind) || string.IsNullOrWhiteSpace(key)) continue;
            db.SyncItems.Add(new SyncItemRow { UserId = userId, Kind = kind, ItemKey = key, Data = (token["data"] ?? new JObject()).ToString(Formatting.None), Revision = ++rev });
        }
        await db.SaveChangesAsync(); await tx.CommitAsync();
        return OkData(new { imported = rev, revision = rev });
    }
    #endregion
    ...
    {
        var admin = await RequireAdmin(true); if (admin.Result != null) return admin.Result;
        if (plugin == null || !Uri.TryCreate(plugin.Url, UriKind.Absolute, out var uri) || uri.Scheme != "https") return Error(400, "invalid_plugin", "Нужен HTTPS URL");
        byte[] script;
        try { script = await PluginHttp.GetByteArrayAsync(uri); }
        catch { return Error(400, "plugin_unreachable", "Не удалось скачать плагин для проверки"); }
        if (script.Length is 0 or > 2 * 1024 * 1024) return Error(400, "invalid_plugin_size", "Размер плагина должен быть до 2 МБ");
    ...
    ActionResult Asset(string name, string type) => File(Encoding.UTF8.GetBytes(System.IO.File.ReadAllText(Path.Combine(ModInit.modpath, "Assets", name), Encoding.UTF8)), type);
